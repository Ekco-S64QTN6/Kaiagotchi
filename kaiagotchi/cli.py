...
def Kaiagotchi_cli():
    ...
    def do_auto_mode(agent):
        ...
        while True:
            try:
                ...
                #
                # From Kaiagotchi's perspective, the more new access points
                # and / or client stations nearby, the longer one epoch of
                # its relative time will take ... basically, in Kaiagotchi's universe,
    ...
    # Kaiagotchi --help
    parser.add_argument('-C', '--config', action='store', dest='config', default='/etc/Kaiagotchi/default.toml',
                        help='Main configuration file.')
    parser.add_argument('-U', '--user-config', action='store', dest='user_config', default='/etc/Kaiagotchi/config.toml',
    ...
    if args.wizard:
        ...
        if pwn_restore in ('y', 'yes'):
            os.system("cp -f /etc/Kaiagotchi/config.toml.bak /etc/Kaiagotchi/config.toml")
            print("Your previous configuration is restored, and I will restart in 5 seconds.")
            time.sleep(5)
            os.system("service Kaiagotchi restart")
        else:
            ...
            if pwn_check.lower() in ('y', 'yes'):
                os.system("mv -f /etc/Kaiagotchi/config.toml /etc/Kaiagotchi/config.toml.bak")
                with open("/etc/Kaiagotchi/config.toml", "a+") as f:
                    f.write("# Do not edit this file if you do not know what you are doing!!!\n\n")
                    # Set Kaiagotchi name
                    print("Welcome to the interactive installation of your personal Kaiagotchi configuration!\n"
                          "My name is Jayofelony, how may I call you?\n\n")
                    pwn_name = input("Kaiagotchi name (no spaces): ")
                    if pwn_name == "":
                        pwn_name = "Kaiagotchi"
                        print("I shall go by Kaiagotchi from now on!")
                        pwn_name = (f"[main]\n"
                                    f"name = \"{pwn_name}\"\n")
                        f.write(pwn_name)
                    else:
                    ...
                    if pwn_display_enabled.lower() in ('y', 'yes'):
                        ...
                        pwn_display_type = input("What display do you use?\n\n"
                                                 ...
                                                 "https://github.com/jayofelony/Kaiagotchi/blob/master/Kaiagotchi/utils.py#L240-L501\n\n"
                                                 "Display type: ")
                        if pwn_display_type != "":
    ...
    if args.check_update:
        resp = requests.get("https://api.github.com/repos/jayofelony/Kaiagotchi/releases/latest")
        latest = resp.json()
        latest_ver = latest['tag_name'].replace('v', '')

        local = version_to_tuple(Kaiagotchi.__version__)
        remote = version_to_tuple(latest_ver)
        if remote > local:
            user_input = input("There is a new version available! Update from v%s to v%s?\n[Y/N] " % (Kaiagotchi.__version__, latest_ver))
            # input validation
            if user_input.lower() in ('y', 'yes'):
                if os.path.exists('/root/.auto-update'):
                    os.system("rm /root/.auto-update && systemctl restart Kaiagotchi")
                else:
    ...
    display = Display(config=config, state={'name': '%s>' % Kaiagotchi.name()})
