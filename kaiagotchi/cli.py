...
# Project logger (defined early for use in import-guard logger calls)
...
try:
    ...
    logger.error("Failed to import core subsystems (Agent/UI/Splash): %s", e, exc_info=True)
    Agent = None
    SplashScreen = None
...
async def run_agent() -> int:
    ...
    def handle_signal():
        ...
        if shutdown_requested > 1:
            logger.warning("Force exiting Kaiagotchi immediately...")
            os._exit(0)
        ...
        if Manager is not None:
            ...
            try:
                ...
                if splash:
                    ...
                    # If splash failed to load, wait briefly for manager startup
                    done, pending = await asyncio.wait(
            ...
            try:
                ...
                if agent:
                    try:
                        ...
                        logger.error(f"Error stopping agent: {stop_err}")
